...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data: accessJob, error: accessError } = await admin
    .from("jobs")
    .select("assigned_technician_id,assigned_to")
    .eq("id", jobId)
    .maybeSingle();
  if (accessError) return NextResponse.json({ error: accessError.message }, { status: 400 });
  if (!accessJob) return NextResponse.json({ error: "Job not found" }, { status: 404 });
  const assignedTechId = accessJob.assigned_technician_id ?? accessJob.assigned_to ?? null;
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data: job } = await admin
    .from("jobs")
    .select("id,client_id,assigned_technician_id,assigned_to")
    .eq("id", jobId)
    .maybeSingle();
  ...
  const assignedTechId = job.assigned_technician_id ?? job.assigned_to ?? null;
  if (!assignedTechId) {
    return NextResponse.json({ error: "No technician assigned to this job yet." }, { status: 400 });
  }
  if (assignedTechId !== userId) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  ...
  for (const file of files.slice(0, 10)) {
    ...
    const storagePath = `${job.client_id}/${jobId}/technician-${Date.now()}-${safeFileName(file.name || "upload.bin")}`;

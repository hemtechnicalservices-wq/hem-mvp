...
export async function GET(req: NextRequest) {
  ...
  const data = (await res.json().catch(() => null)) as { dashboard?: Record<string, unknown>; analytics?: Record<string, unknown>; error?: string } | null;
  if (!res.ok) return NextResponse.json(data ?? { error: "Failed" }, { status: res.status });
  return NextResponse.json({ jobs: { dashboard: data?.dashboard ?? {}, analytics: data?.analytics ?? {} } });
}
